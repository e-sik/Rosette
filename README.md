...
- **[Backtesting.py](https://kernc.github.io/backtesting.py/)**: The core execution engine. It provides the `Strategy` classes and ultra-fast vectorized backtesting logic that powers every historical run.
...
├── results/              # Directory where all stats, plots, and bulk results are exported
└── refernce/             # Reference codes & Jupyter notebooks (Do Not Delete)
```
...
The entire workflow is driven via the browser dashboard. Once your environment is active, start the Streamlit server:
...
2. **Strategy Editor (Tab 2)**: Write your strategy using the integrated full-featured IDE (syntax highlighting, Vim/Emacs keybindings, adjustable font size, and visual themes).
...
4. **Results & Analytics (Tab 4)**: Review your saved single run and bulk test results in one consolidated view.
   - **Run Performance & Comparison**: Select multiple past runs using the multi-selector to view their performance metrics side-by-side in a comparative table. Select an individual run to render its saved interactive HTML chart natively within the dashboard.
   - **Monte Carlo Analysis**: Embedded directly within the single and comparison run views. Stress-test your strategy by simulating thousands of different trade sequences. View an interactive 'Spaghetti Plot' of all simulated equity curves to understand potential drawdowns visually, and check the automated **Comparative Verdict** to see if your initial backtest was "lucky".
   - **Bulk Portfolio & Matrix Viewer**: Read, view, and analyze bulk test runs. It processes the combined trade ledger to treat the entire bulk run as a single unified portfolio.
5. **Bulk Testing (Tab 5)**: Automatically evaluate a strategy across hundreds of discrete timeframes or candle intervals.
   ...
   - Results are automatically exported to the consolidated `results/` directory with a `BULK_` prefix.
...
- **SaaS Migration**: Integrate authentication and block storage to support multiple users natively on cloud hosting.
